...
housing1b_colnames <-
  c("select_characteristics", "total", "occupied_without_rent", "yes_caughtup", "no_notcaughtup", "rent_deferred", "did_not_report", "did_not_report_tenure")
...
download.file(paste0
                ("https://www2.census.gov/programs-surveys/demo/tables/hhp/2020/wk",week,"/housing1b_week",week,".xlsx"), 
                temp.file, mode = "wb")

housing1b_data <- read_excel(temp.file, sheet=paste0(state_abbr),skip = 4) %>%
  ...
  #convert numeric variables to numeric
    mutate_at(vars(total,occupied_without_rent,yes_caughtup,
                   no_notcaughtup,rent_deferred,did_not_report,did_not_report_tenure),
              list(~ as.numeric(.))) %>%
  ...
  rowwise() %>%
  mutate(notcaughtup_numerator=sum(no_notcaughtup,rent_deferred,na.rm=TRUE),
         notcaughtup_denominator=sum(yes_caughtup,no_notcaughtup,rent_deferred,na.rm=TRUE)) %>%
...
temp.file <- paste(tempfile(),".xlsx",sep = "")
download.file(paste0
                ("https://www2.census.gov/programs-surveys/demo/tables/hhp/2020/wk",week,"/housing1b_se_week",week,".xlsx"), 
                temp.file, mode = "wb")

housing1b_data_se <- read_excel(temp.file, sheet=paste0(state_abbr),skip = 4) %>%
    set_names(housing1b_colnames) %>%
  ...
  rename(total_se=total,
         ...
         rent_deferred_se=rent_deferred,
         did_not_report_se=did_not_report,
         did_not_report_tenure_se=did_not_report_tenure) %>%
  
  #calculate numerator, denominator se's
  mutate(notcaughtup_numerator_se=sqrt((no_notcaughtup_se^2)+(rent_deferred_se^2)),
         notcaughtup_denominator_se=sqrt((yes_caughtup_se^2)+(no_notcaughtup_se^2)+(rent_deferred_se^2))) 
