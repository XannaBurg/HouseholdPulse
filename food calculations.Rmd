...
food_4weeks_data <- dbGetQuery(con,food_sql) %>%
  ...
  mutate(percent_se=(1/totalvalidresponses)*(sqrt((foodinsufficient_numerator_se^2)-(percent*(totalvalidresponses_se^2))))) %>%
  mutate(percent_se=if_else(is.na(percent_se),(1/totalvalidresponses)*(sqrt((foodinsufficient_numerator_se^2)+(percent*(totalvalidresponses_se^2)))),percent_se)) %>%
...
food_4weeks_data_overallfree2 <- food_4weeks_data_overallfree %>%
  ...
  mutate(percent=count_sums/denominator_overall,
         percent_se=(1/denominator_overall)*(sqrt((count_sums_se^2)-(percent*(denominator_overall_se^2))))) %>%
  mutate(percent_se=if_else(is.na(percent_se),(1/denominator_overall)*(sqrt((count_sums_se^2)+(percent*(denominator_overall_se^2)))),percent_se)) %>%
  mutate(percent_lci=percent-(percent_se*1.645)) %>%
  mutate(percent_uci=percent+(percent_se*1.645)) %>%
...
food_4weeks_data_unaffordable <- unaffordable_4weeks_data %>%
  ...
  mutate(percent=children_not_eating/totalhouseholdswithchildren,
         percent_se=(1/totalhouseholdswithchildren)*(sqrt((children_not_eating_se^2)-(percent*(totalhouseholdswithchildren_se^2))))) %>%
  mutate(percent_se=if_else(is.na(percent_se),(1/totalhouseholdswithchildren)*(sqrt((children_not_eating_se^2)+(percent*(totalhouseholdswithchildren_se^2)))),percent_se)) %>%
  mutate(percent_lci=percent-(percent_se*1.645)) %>%
  mutate(percent_uci=percent+(percent_se*1.645)) %>%
